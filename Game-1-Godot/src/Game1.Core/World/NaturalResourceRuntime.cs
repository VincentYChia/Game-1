...
public sealed class NaturalResourceRuntime
{
    ...
    /// <summary>natural_resource.py fallback loot map — the EFFECTIVE dict.
    /// Python's "legacy alias" attribute names resolve to the SAME id
    /// strings as canonical entries, so the literal collapses last-writer-
    /// wins to 29 keys (verifier finding, confirmed by executing the real
    /// dict): copper_vein/iron_deposit/mithril_cache/limestone_outcrop/
    /// granite_formation/obsidian_flow carry the alias tuples, and no
    /// copper_ore/iron_ore/... keys exist at all.</summary>
    private static readonly Dictionary<string, (string Item, int Min, int Max)> FallbackLoot = new()
    {
        ...
        ["copper_vein"] = ("copper_ore", 1, 3), ["iron_deposit"] = ("iron_ore", 1, 3),
        ["tin_seam"] = ("tin_ore", 2, 4), ["steel_node"] = ("steel_ore", 2, 4),
        ["mithril_cache"] = ("mithril_ore", 2, 5), ["adamantine_lode"] = ("adamantine_ore", 1, 2),
        ["orichalcum_trove"] = ("orichalcum_ore", 1, 2), ["etherion_nexus"] = ("etherion_ore", 1, 2),
        ["limestone_outcrop"] = ("limestone", 1, 2), ["granite_formation"] = ("granite", 1, 2),
        ["shale_bed"] = ("shale", 3, 5), ["basalt_column"] = ("basalt", 2, 4),
        ["marble_quarry"] = ("marble", 2, 4), ["quartz_cluster"] = ("crystal_quartz", 2, 4),
        ["obsidian_flow"] = ("obsidian", 2, 3), ["voidstone_shard"] = ("voidstone", 1, 2),
