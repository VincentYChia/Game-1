...
- **v8.3** (July 17, 2026): **Playtest-GO pass** (crux-foundry branch; findings F1-F20 in `crux-foundry/FINDINGS.md`; consolidated report `crux-foundry/SHIP_READINESS_REPORT.md`). Combat conformance on the action path (enemy DEF was a dead stat; crit unified; LCK retuned 0.02→0.12 per optimizer; VIT regen / DEF armor-eff / INT elemental wired). WMS: async L2 narrative upgrades (no LLM on the game loop), context budgets enforced at assembly, response parser rewritten against real model output (significance/severity contract, fences, tag allow-list). Real-LLM certification of every Haiku task — found+fixed Haiku 4.5's temperature+top_p 400 in BOTH API clients (every game-path Claude call was broken since the June model swap; fixtures masked it). WES hub deep-work: output format actually delivered to the model, stale "do not plan quests" awareness fixed, retry, live + co-emitted registry context in slices, tolerant element-children XML dialect, deterministic registry-dedup guard; certified 8/8 at three fidelities (subagent roleplay / Haiku / gemma3:4b + qwen3:4b). Faction/NPC affinity wired end-to-end: quest turn-in moves affinity, NPC memory persists (hydrate/flush pump), WNS AffinityShift bridge fixed. New designer tools: `tools/prompt_coverage_slideshow.py` (150/150 fragments in 36 slides), `tools/hub_example_set_apply.py` + `tools/hub_cert_harness.py` (leak-tested hub example sets: frostpeak applied / schematic / moors_original). 1,219 tests green. **Playtest blocker: replace the dead ANTHROPIC_API_KEY env var** (it 401s and shadows `.env`).
