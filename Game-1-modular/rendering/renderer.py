...
class Renderer:
    ...
    def render_world(self, world: WorldSystem, camera: Camera, character: Character,
        ...
        for entity in world.get_visible_placed_entities(camera.position, Config.VIEWPORT_WIDTH, Config.VIEWPORT_HEIGHT):
            ...
            elif entity.entity_type == PlacedEntityType.DROPPED_ITEM:
                # Dropped items - try to find icon from material or equipment database
                from data.databases.material_db import MaterialDatabase
                from data.databases.equipment_db import EquipmentDatabase
                mat_db = MaterialDatabase.get_instance()
                equip_db = EquipmentDatabase.get_instance()

                mat = mat_db.get_material(entity.item_id)
            ...
            # Render tier indicator (skip for dropped items)
            if entity.entity_type != PlacedEntityType.DROPPED_ITEM:
                ...
                # For dropped items, show quantity badge
                quantity = 1
                if entity.effect_params and 'quantity' in entity.effect_params:
                    quantity = entity.effect_params['quantity']
                if quantity > 1:
                    ...
                    qty_x = sx + size // 2 - qty_surf.get_width() - 4
                    qty_y = sy + size // 2 - qty_surf.get_height()
                    self.screen.blit(qty_bg, (qty_x - 2, qty_y))
                    self.screen.blit(qty_surf, (qty_x, qty_y))
        ...
        if hasattr(world, 'spawn_storage_chest') and world.spawn_storage_chest:
            ...
            # Try to load chest icon
            icon = image_cache.get_image("devices/storage_chest.png", (size, size))
            if icon:
                icon_rect = icon.get_rect(center=(sx, sy))
                self.screen.blit(icon, icon_rect)
            else:
                # Fallback: draw a chest shape
                rect = pygame.Rect(sx - size // 2, sy - size // 2, size, size)
                chest_color = (100, 150, 200) if in_range else (60, 100, 140)
                pygame.draw.rect(self.screen, chest_color, rect)
                pygame.draw.rect(self.screen, (40, 80, 120), rect, 2)
                # Draw chest lid
                lid_rect = pygame.Rect(sx - size // 2, sy - size // 2, size, size // 3)
                pygame.draw.rect(self.screen, (80, 130, 180), lid_rect)
                pygame.draw.rect(self.screen, (40, 80, 120), lid_rect, 1)
                # Draw chest lock/keyhole
                pygame.draw.circle(self.screen, (200, 180, 100), (sx, sy - size // 6), size // 8)

            # Render hint if in range
            if in_range:
                hint_surf = self.tiny_font.render("Click to open", True, (200, 220, 255))
        ...
        # Render enemies — readable with names, facing, state, telegraphs
        import math as _emath
        import time as _etime
        image_cache = ImageCache.get_instance()
        try:
            from animation.swing_renderer import get_swing_renderer as _get_eswing
            _enemy_swing = _get_eswing()
        except ImportError:
            _enemy_swing = None
        ...
        def _enemy_tag_color(tags, fallback=(220, 220, 240)):
            for t in tags:
                if t in _ETAG_COLORS:
                    return _ETAG_COLORS[t]
            return fallback
            ...
            for enemy in combat_manager.get_all_active_enemies():
                ...
                if enemy.is_alive:
                    ...
                    if anim_timer > 0:
                        ...
                        if arc_r > 2 and _enemy_swing:
                            try:
                                if _atk_shape == 'circle':
                                    frames = _enemy_swing.get_enemy_radial_frames(active_color, arc_r)
                                elif _atk_shape == 'line':
                                    frames = _enemy_swing.get_enemy_thrust_frames(active_color, arc_r)
                                else:
                                    frames = _enemy_swing.get_enemy_swing_frames(
                                        active_color, _enemy_arc_deg, arc_r)

                                # Map anim_progress to frame index
                                progress = min(1.0, anim_progress * 1.4)
                                frame_idx = min(len(frames) - 1,
                                                int(progress * (len(frames) - 1)))
                                frame = frames[frame_idx]

                                if _atk_shape in ('arc', 'circle'):
                                    # Arc/circle: rotate frame to attack facing angle
                                    rotated = pygame.transform.rotate(frame, -angle)
                                    rc = rotated.get_rect(center=(ex, ey))
                                    self.screen.blit(rotated, rc)
                                else:
                                    # Line/thrust: rotate frame to facing
                                    rotated = pygame.transform.rotate(frame, -angle)
                                    rc = rotated.get_rect(center=(ex, ey))
                                    self.screen.blit(rotated, rc)
                            except Exception:
                                ...
                                ac_ = as_size // 2
                                fade_alpha = max(0, int(150 * (1.0 - anim_progress)))
                                pygame.draw.circle(asurf, (*active_color, fade_alpha),
                                                   (ac_, ac_), arc_r, 3)
                                self.screen.blit(asurf, (ex - ac_, ey - ac_))
                        elif arc_r > 2:
                            # No swing renderer — simple fallback
                            as_size = arc_r * 2 + 4
                            asurf = pygame.Surface((as_size, as_size), pygame.SRCALPHA)
                            ac_ = as_size // 2
                            fade_alpha = max(0, int(150 * (1.0 - anim_progress)))
                    ...
                    if icon:
                        ...
                        if cat == 'ooze':
                            ...
                            # Beast: ear nubs on top
                            pygame.draw.circle(self.screen, enemy_color,
                                               (ex - size // 3, ey - size + 2), max(2, size // 4))
                            pygame.draw.circle(self.screen, enemy_color,
                                               (ex + size // 3, ey - size + 2), max(2, size // 4))
                        elif cat == 'undead':
                            # Undead: hollow eyes
                            eye_r = max(1, size // 5)
                            pygame.draw.circle(self.screen, (0, 0, 0), (ex - size // 3, ey - size // 4), eye_r)
                            pygame.draw.circle(self.screen, (0, 0, 0), (ex + size // 3, ey - size // 4), eye_r)
                        elif cat == 'elemental':
                            # Elemental: rotating particle dots
                            t = _etime.time() * 2
                            for j in range(3):
                                a = t + j * 2.094  # 120 degrees apart
                                px = ex + int(_emath.cos(a) * size * 0.7)
                                py = ey + int(_emath.sin(a) * size * 0.7)
                                pygame.draw.circle(self.screen, (255, 255, 200), (px, py), max(1, size // 6))

                    # --- Facing direction indicator (small triangle showing where enemy looks) ---
                    facing = getattr(enemy, 'facing_angle', 0.0)
                    facing_rad = _emath.radians(facing)
                    ind_dist = size + 4
                    ind_x = ex + int(_emath.cos(facing_rad) * ind_dist)
                    ind_y = ey + int(_emath.sin(facing_rad) * ind_dist)
                    # Small triangle
                    tri_size = max(2, size // 4)
                    perp = facing_rad + _emath.pi / 2
                    p1 = (ind_x + int(_emath.cos(facing_rad) * tri_size),
                          ind_y + int(_emath.sin(facing_rad) * tri_size))
                    p2 = (ind_x + int(_emath.cos(perp) * tri_size // 2),
                          ind_y + int(_emath.sin(perp) * tri_size // 2))
                    p3 = (ind_x - int(_emath.cos(perp) * tri_size // 2),
                          ind_y - int(_emath.sin(perp) * tri_size // 2))
                    pygame.draw.polygon(self.screen, enemy_color, [p1, p2, p3])

                    # --- AI state dot (tiny colored circle indicating intent) ---
                    state_name = getattr(enemy.ai_state, 'value', 'idle')
                    state_color = _STATE_COLORS.get(state_name, (150, 150, 150))
                    pygame.draw.circle(self.screen, state_color, (ex + size + 2, ey - size - 2), 3)
                    pygame.draw.circle(self.screen, (0, 0, 0), (ex + size + 2, ey - size - 2), 3, 1)
                    ...
                    hp_color = (0, 200, 0) if health_percent > 0.5 else (255, 200, 0) if health_percent > 0.25 else (255, 60, 60)
                    pygame.draw.rect(self.screen, hp_color, (ex - bar_w // 2, bar_y, hp_w, bar_h))

                    # --- Name + tier label ---
                    name = enemy.definition.name
                    tier_prefix = f"T{enemy.definition.tier} " if not enemy.is_boss else "BOSS "
                    label = f"{tier_prefix}{name}"
                    name_surf = self.tiny_font.render(label, True, (255, 255, 255))
                    name_bg = pygame.Surface((name_surf.get_width() + 4, name_surf.get_height() + 2), pygame.SRCALPHA)
                    name_bg.fill((0, 0, 0, 120))
                    self.screen.blit(name_bg, (ex - name_surf.get_width() // 2 - 2, bar_y - name_surf.get_height() - 2))
                    self.screen.blit(name_surf, (ex - name_surf.get_width() // 2, bar_y - name_surf.get_height() - 1))
    ...
    def _render_action_combat(self, camera: Camera, character, combat_manager, ac: dict):
        ...
        # --- Enemy windup telegraphs (show sweep area with intensifying hue) ---
        if combat_manager:
            for enemy in combat_manager.get_all_active_enemies():
        ...
        # --- Player windup telegraph (attack arc preview, moves with player) ---
        if player_sm and player_sm.is_in_windup and player_sm.current_attack:
            attack_def = player_sm.current_attack
            progress = player_sm.windup_progress
            px, py = camera.world_to_screen(character.position)
            facing_angle = getattr(character, 'facing_angle', 0.0)
            facing_rad = math.radians(facing_angle)

            # Get element color from attack context
            ctx = player_sm.damage_context
            effect_tags = ctx.get('effect_tags', []) if ctx else []
        ...
        if hitbox_sys:
            # Lazy-load weapon visual resolver and swing renderer
            _weapon_style = None
            _swing_renderer = None
            try:
                from animation.weapon_visuals import resolve_weapon_visual
                from animation.swing_renderer import get_swing_renderer
                _weapon_style = True
                _swing_renderer = get_swing_renderer()
            except ImportError:
                _weapon_style = None
            ...
            for hitbox in hitbox_sys.active_hitboxes:
                ...
                if hitbox.definition.shape == "circle":
                    ...
                    if radius_px > 1:
                        if is_player and _swing_renderer and _weapon_style:
                            # Frame-based radial burst
                            try:
                                frames = _swing_renderer.get_radial_frames(style, radius_px)
                    ...
                    if radius_px > 2:
                        if is_player and _swing_renderer and _weapon_style:
                            # Frame-based swing arc
                            try:
                                frames = _swing_renderer.get_swing_frames(style, radius_px)
                                progress = min(1.0, (1.0 - life_ratio) * 1.4)
                                frame_idx = min(len(frames) - 1, int(progress * (len(frames) - 1)))
                                frame = frames[frame_idx]
                                # Frames are rendered pointing RIGHT (0°).
                                # Rotate to match attack facing angle.
                    ...
                    if is_player and _swing_renderer and _weapon_style:
                        # Frame-based thrust
                        try:
                            frames = _swing_renderer.get_thrust_frames(style, length_px)
                            progress = min(1.0, (1.0 - life_ratio) * 1.4)
                            frame_idx = min(len(frames) - 1, int(progress * (len(frames) - 1)))
                            frame = frames[frame_idx]
                            # Frames point RIGHT. Rotate to facing angle.
                            rotated = pygame.transform.rotate(frame, -hitbox.facing_angle)
                            rc = rotated.get_rect(center=(hx, hy))
                            self.screen.blit(rotated, rc)
        ...
        # --- Projectiles (tag-driven visual shapes, polished with glow + trails) ---
        if projectile_sys:
            for proj in projectile_sys.projectiles:
                ...
                if vis:
                    ...
                    if shape == 'elongated':
                        # Arrow/shard with motion blur trail + glow halo
                        length = vis.get('length_px', 8)
                        width = vis.get('width_px', 3)
                        # Larger surface for glow halo
                        pad = 6
                        surf_w = length + pad * 2
                        surf_h = width + pad * 2
                        arrow_surf = pygame.Surface((surf_w, surf_h), pygame.SRCALPHA)
                        # Outer glow halo (3-pass)
                        pygame.draw.ellipse(arrow_surf, (*glow_color, 20),
                                            (0, 0, surf_w, surf_h))
                        pygame.draw.ellipse(arrow_surf, (*glow_color, 40),
                                            (pad // 2, pad // 2, surf_w - pad, surf_h - pad))
                        # Core body
                        pygame.draw.ellipse(arrow_surf, color,
                                            (pad, pad, length, width))
                        # White highlight center
                        hl_w = max(2, length // 2)
                        hl_h = max(1, width // 3)
                        pygame.draw.ellipse(arrow_surf, (255, 255, 255, 180),
                                            (pad + length // 4, pad + width // 3, hl_w, hl_h))
                        ...
                        # Motion blur trail behind projectile
                        trail_len = 18
                        trail_sx = px_screen - int(math.cos(facing_rad) * trail_len)
                        trail_sy = py_screen - int(math.sin(facing_rad) * trail_len)
                        for tl in range(3):
                            t_frac = (tl + 1) / 4.0
                            tx = int(px_screen + (trail_sx - px_screen) * t_frac)
                            ty = int(py_screen + (trail_sy - py_screen) * t_frac)
                            t_alpha = int(60 * (1.0 - t_frac))
                            t_size = max(1, int(width * (1.0 - t_frac * 0.5)))
                            trail_surf = pygame.Surface((t_size * 2 + 2, t_size * 2 + 2), pygame.SRCALPHA)
                            pygame.draw.circle(trail_surf, (*color, t_alpha),
                                               (t_size + 1, t_size + 1), t_size)
                            self.screen.blit(trail_surf, (tx - t_size - 1, ty - t_size - 1))

                    elif shape == 'beam':
                        # Beam with multi-layer glow and bright tip
                        bw = max(8, vis.get('width_px', 8))
                        trail_len = 32
                        sx = px_screen - int(math.cos(facing_rad) * trail_len)
                        sy = py_screen - int(math.sin(facing_rad) * trail_len)
                        # Wide outer glow (lowest alpha)
                        glow_surf = pygame.Surface(
                            (Config.VIEWPORT_WIDTH, Config.VIEWPORT_HEIGHT), pygame.SRCALPHA)
                        pygame.draw.line(glow_surf, (*glow_color, 30),
                                         (sx, sy), (px_screen, py_screen), bw + 10)
                        pygame.draw.line(glow_surf, (*glow_color, 60),
                                         (sx, sy), (px_screen, py_screen), bw + 4)
                        # Core beam
                        pygame.draw.line(glow_surf, (*color, 220),
                                         (sx, sy), (px_screen, py_screen), bw)
                        # Bright center line
                        pygame.draw.line(glow_surf, (255, 255, 255, 180),
                                         (sx, sy), (px_screen, py_screen), max(2, bw // 3))
                        # Bright tip with halo
                        pygame.draw.circle(glow_surf, (255, 255, 255, 220),
                                           (px_screen, py_screen), bw // 2 + 2)
                        pygame.draw.circle(glow_surf, (*bright_color, 80),
                                           (px_screen, py_screen), bw // 2 + 6)
                        self.screen.blit(glow_surf, (0, 0))

                    else:  # 'orb' default — with glow halo + white highlight
                        ...
                        hc = halo_r + 2
                        # Outermost soft glow
                        pygame.draw.circle(halo_surf, (*glow_color, 20), (hc, hc), halo_r)
                        # Middle glow
                        pygame.draw.circle(halo_surf, (*glow_color, 50),
                                           (hc, hc), int(halo_r * 0.7))
                        # Core body
                        pygame.draw.circle(halo_surf, (*color, 230), (hc, hc), radius)
                        # White-hot center highlight
                        hl_r = max(1, radius // 2)
                        pygame.draw.circle(halo_surf, (255, 255, 255, 200), (hc, hc), hl_r)
                        self.screen.blit(halo_surf,
                                         (px_screen - hc, py_screen - hc))
                        # Motion blur trail
                        trail_len = 12
                        for tl in range(3):
                            t_frac = (tl + 1) / 4.0
                            tx = int(px_screen - math.cos(facing_rad) * trail_len * t_frac)
                            ty = int(py_screen - math.sin(facing_rad) * trail_len * t_frac)
                            t_alpha = int(50 * (1.0 - t_frac))
                            t_size = max(1, int(radius * (1.0 - t_frac * 0.4)))
                            trail_surf = pygame.Surface((t_size * 2 + 2, t_size * 2 + 2), pygame.SRCALPHA)
                            pygame.draw.circle(trail_surf, (*color, t_alpha),
                                               (t_size + 1, t_size + 1), t_size)
                            self.screen.blit(trail_surf, (tx - t_size - 1, ty - t_size - 1))
                else:
                    # Fallback: simple circle with tag color + basic glow
                    color = _color_from_tags(proj_tags, (200, 200, 255))
                    radius = max(3, int(proj.definition.hitbox_radius * Config.TILE_SIZE * 0.5))
                    # Small glow halo
                    glow_surf = pygame.Surface((radius * 4 + 4, radius * 4 + 4), pygame.SRCALPHA)
                    gc = radius * 2 + 2
                    pygame.draw.circle(glow_surf, (*color, 30), (gc, gc), radius * 2)
                    self.screen.blit(glow_surf, (px_screen - gc, py_screen - gc))
    ...
    def render_spawn_chest_ui(self, chest) -> Tuple[pygame.Rect, List[Tuple[pygame.Rect, int]]]:
        """Render the spawn storage chest UI for item storage.
        ...
        # Chest UI position (left side of screen, above inventory)
        ui_width = 280
        ui_height = 320
        ui_x = 20
        ui_y = Config.VIEWPORT_HEIGHT - Config.INVENTORY_PANEL_HEIGHT - ui_height - 20

        # Background panel (blue tint for storage chest)
        chest_rect = pygame.Rect(ui_x, ui_y, ui_width, ui_height)
        panel_surf = pygame.Surface((ui_width, ui_height), pygame.SRCALPHA)
        panel_surf.fill((20, 40, 60, 240))
        self.screen.blit(panel_surf, (ui_x, ui_y))

        # Border
        pygame.draw.rect(self.screen, (80, 140, 200), chest_rect, 3)

        # Title
        title_surf = self.font.render("Storage Chest", True, (100, 200, 255))
        self.screen.blit(title_surf, (ui_x + (ui_width - title_surf.get_width()) // 2, ui_y + 10))

        # Subtitle
        subtitle_surf = self.tiny_font.render("Click to retrieve, Q to store", True, (180, 200, 220))
        self.screen.blit(subtitle_surf, (ui_x + (ui_width - subtitle_surf.get_width()) // 2, ui_y + 35))
        ...
        for idx, (item_id, quantity) in enumerate(chest.contents):
            ...
            # Slot background
            pygame.draw.rect(self.screen, (40, 60, 80), slot_rect)
            pygame.draw.rect(self.screen, (80, 120, 160), slot_rect, 2)
            ...
            if icon:
                ...
                # Fallback: colored square
                pygame.draw.rect(self.screen, (80, 120, 160), (slot_x + 4, slot_y + 4, slot_size - 8, slot_size - 8))
    ...
    def _render_attack_effects(self, camera: 'Camera'):
        ...
        try:
            ...
            try:
                from animation.swing_renderer import get_swing_renderer as _get_fx_swing
                _fx_swing = _get_fx_swing()
            except ImportError:
                _fx_swing = None
            ...
            for effect in manager.get_active_effects():
                ...
                if effect.effect_type == AttackEffectType.SLASH_ARC:
                    ...
                    if _fx_swing:
                        try:
                            arc_deg = getattr(effect, 'arc_degrees', 65.0)
                            frames = _fx_swing.get_enemy_swing_frames(rgb, arc_deg, radius_px)
                            frame_idx = min(len(frames) - 1,
                                            int(progress * (len(frames) - 1)))
                            frame = frames[frame_idx]
                            # Apply overall alpha fade from effect
                            if alpha < 200:
                                frame = frame.copy()
                                frame.set_alpha(alpha)
                            rotated = pygame.transform.rotate(frame, -effect.facing_angle)
                            rc = rotated.get_rect(center=(start_sx, start_sy))
                            self.screen.blit(rotated, rc)
                        except Exception:
                            ...
                            arc_surf = pygame.Surface((surf_size, surf_size), pygame.SRCALPHA)
                            center = surf_size // 2
                            pygame.draw.circle(arc_surf, (*rgb, min(255, alpha // 2)),
                                               (center, center), radius_px, 3)
                            self.screen.blit(arc_surf, (start_sx - center, start_sy - center))
                    else:
                        # No swing renderer — simple arc indicator
                        surf_size = radius_px * 2 + 8
                        arc_surf = pygame.Surface((surf_size, surf_size), pygame.SRCALPHA)
                        center = surf_size // 2
                        pygame.draw.circle(arc_surf, (*rgb, min(255, alpha // 2)),
                    ...
                    if _fx_swing:
                        try:
                            frames = _fx_swing.get_enemy_thrust_frames(rgb, length_px)
                            frame_idx = min(len(frames) - 1,
                                            int(progress * (len(frames) - 1)))
                            frame = frames[frame_idx]
                            if alpha < 200:
                            ...
                            pygame.draw.line(self.screen, (*rgb, min(255, alpha)),
                        ...
                        glow_surf = pygame.Surface(
                            (Config.VIEWPORT_WIDTH, Config.VIEWPORT_HEIGHT), pygame.SRCALPHA)
                        pygame.draw.line(glow_surf, (*rgb, min(255, int(alpha * 0.4))),
                                        (start_sx, start_sy), (ex, ey), 8)
                        pygame.draw.line(glow_surf, (*rgb, min(255, int(alpha * 0.85))),
                                        (start_sx, start_sy), (ex, ey), 4)
                        pygame.draw.circle(glow_surf, (*rgb, min(255, int(alpha * 0.95))),
                                          (ex, ey), 6)
                        self.screen.blit(glow_surf, (0, 0))
