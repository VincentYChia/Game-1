...
import random
import math
from typing import Any, Dict, List, Set, Tuple, Optional, ClassVar, TYPE_CHECKING
...
class Chunk:
    ...
    def _determine_chunk_type(self) -> str:
        ...
        Geographic dispatch is JSON-driven via
        ``world_system/config/geo_chunk_dispatch.json`` (loaded by
        :class:`ChunkTemplateDatabase`). The database also honors
        ``geoTypes`` arrays declared on individual chunk templates so
        WES-generated biomes can self-register their own bridge entries
        without editing the dispatch file.
        """
        ...
        if self._geographic_data is not None:
            ...
            template = template_db.get_for_geo_type(ct_value)
            if template is not None:
                return template.chunk_type
            # No dispatch entry — degrade to a safe default and surface
            # the gap so designers can update the dispatch JSON.
            print(
                f"[Chunk] No geo_chunk_dispatch entry for '{ct_value}' "
                f"— defaulting to peaceful_forest. Update "
                f"world_system/config/geo_chunk_dispatch.json or add "
                f"a geoTypes entry on a chunk template."
            )
    ...
    def spawn_resources(self):
        ...
        # Preferred path: chunk template's resourceDensity defines exactly
        # which resources spawn here and at what relative weight. This is
        # the JSON-driven pathway WES-generated chunk templates land on.
        template_db = ChunkTemplateDatabase.get_instance()
        template = template_db.get(self.chunk_type)
        if (
            template is not None
            and template.resource_density
            and self._spawn_from_template(
                template, start_x, start_y, resource_count, tier_range, occupied_positions
            )
        ):
            return

        # Get resource candidates from database (substring-matched fallback
        # for chunks without a template, e.g. legacy chunks generated by the
        # old BiomeGenerator code path).
    ...
    def _spawn_from_template(
        ...
        """Spawn resources using a ``ChunkTemplate.resource_density`` map.

        Returns True when at least one resource was placed (or no positions
        were available); the caller takes that as a signal not to fall
        through to legacy paths.

        Returns False only when the template's resourceDensity references
        zero resources that exist in :class:`ResourceNodeDatabase` — that
        means every entry was orphaned, in which case the caller should
        try the substring-matched fallback path so the chunk isn't empty.
        """
        db = self.get_resource_db()
        if not db.loaded:
            return False

        # Build (resource_node_def, density_weight) pairs from the template.
        ...
        weighted_pool: List[Tuple[Any, float]] = []
        for spec in template.spawn_pool_resources():
            node_def = db.get_node(spec.resource_id)
            if node_def is None:
                continue
            weighted_pool.append((node_def, spec.spawn_weight))
        ...
        for _ in range(resource_count):
            ...
            # Each slot rolls a max tier within range; only resources at
            # or below that tier are eligible. Mirrors legacy behavior so
            # tier_range from world_generation.JSON keeps controlling the
            # ceiling regardless of which spawn pathway runs.
            tier_cap = min(self._rng.randint(*tier_range), 4)
            eligible = [
                (node_def, weight)
                for (node_def, weight) in weighted_pool
                if node_def.tier <= tier_cap
            ]
            if not eligible:
                continue

            nodes = [n for (n, _w) in eligible]
            weights = [w for (_n, w) in eligible]
            node_def = self._rng.choices(nodes, weights=weights, k=1)[0]
            self.resources.append(
                NaturalResource(pos, node_def.resource_id, node_def.tier)
            )
