...
class Layer5Manager:
    ...
    def __init__(self):
        ...
        # Optional Layer 6 callback (wired by WorldMemorySystem)
        self._layer6_callback: Optional[Callable[[Dict[str, Any]], None]] = None
    ...
    def set_layer6_callback(self, callback) -> None:
        """Register a callback invoked when L5 events are stored.

        The callback receives the stored L5 event dict (as it would
        appear from LayerStore). Used by Layer6Manager to track
        per-nation triggers.
        """
        self._layer6_callback = callback
    ...
    def _store_summary(
        ...
        if self._layer6_callback:
            try:
                ...
                self._layer6_callback(l5_event_dict)
            except Exception as e:
                print(f"[Layer5] Layer 6 callback error: {e}")
