"""Crafting Smithing Evaluator — narrates smithing craft counts."""
...
class CraftingSmithingEvaluator(PatternEvaluator):

    RELEVANT_TYPES = {"craft_attempted"}

    def __init__(self):
        cfg = get_evaluator_config("crafting_smithing")
    ...
    def is_relevant(self, event: WorldMemoryEvent) -> bool:
        if event.event_type not in self.RELEVANT_TYPES:
            return False
        return event.context.get("discipline") == "smithing"
    ...
    def evaluate(self, trigger_event: WorldMemoryEvent,
        ...
        smithing_events = [e for e in recent if e.context.get("discipline") == "smithing"]
        count = len(smithing_events)
        ...
        if qualities:
            ...
            narrative = (
                f"Player has crafted {count} items in smithing, "
                f"{quality_count} at {best_quality} quality."
            )
        else:
            narrative = f"Player has crafted {count} items in smithing."
        ...
        return InterpretedEvent.create(
            ...
            affects_tags=["domain:smithing", "event:crafting", "type:player"],
        )
