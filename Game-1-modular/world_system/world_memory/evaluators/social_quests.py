"""Social Quests Evaluator — narrates quest acceptance, completion, and failure events."""
...
class SocialQuestsEvaluator(PatternEvaluator):

    RELEVANT_TYPES = {"quest_accepted", "quest_completed", "quest_failed"}

    def __init__(self):
        cfg = get_evaluator_config("social_quests")
        ...
        self.significant_min = t.get("significant_min", 10)
        self.major_min = t.get("major_min", 25)
        templates = cfg.get("narrative_templates", {})
        self.tpl_accepted = templates.get("quest_accepted",
            "Player has accepted {count} quests.")
        self.tpl_completed = templates.get("quest_completed",
            "Player has completed {count} quests.")
        self.tpl_failed = templates.get("quest_failed",
            "Player has failed {count} quests.")
    ...
    def evaluate(self, trigger_event: WorldMemoryEvent,
        ...
        if et == "quest_accepted":
            narrative = self.tpl_accepted.format(count=count)
        elif et == "quest_completed":
            narrative = self.tpl_completed.format(count=count)
        else:
            narrative = self.tpl_failed.format(count=count)
