"""Crafting Enchanting Evaluator — narrates enchanting craft counts."""
...
class CraftingEnchantingEvaluator(PatternEvaluator):

    RELEVANT_TYPES = {"craft_attempted"}

    def __init__(self):
        cfg = get_evaluator_config("crafting_enchanting")
    ...
    def is_relevant(self, event: WorldMemoryEvent) -> bool:
        if event.event_type not in self.RELEVANT_TYPES:
            return False
        return event.context.get("discipline") == "enchanting"
    ...
    def evaluate(self, trigger_event: WorldMemoryEvent,
        ...
        enchanting_events = [e for e in recent if e.context.get("discipline") == "enchanting"]
        count = len(enchanting_events)
        ...
        if qualities:
            ...
            narrative = (
                f"Player has crafted {count} items in enchanting, "
                f"{quality_count} at {best_quality} quality."
            )
        else:
            narrative = f"Player has crafted {count} items in enchanting."
        ...
        return InterpretedEvent.create(
            ...
            affects_tags=["domain:enchanting", "event:crafting", "type:player"],
        )
