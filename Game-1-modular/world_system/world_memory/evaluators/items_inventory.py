"""Items Inventory Evaluator — narrates per-item acquisition and consumption counts."""
...
class ItemsInventoryEvaluator(PatternEvaluator):

    RELEVANT_TYPES = {"item_acquired", "item_consumed"}

    def __init__(self):
        cfg = get_evaluator_config("items_inventory")
        ...
        self.tpl_acquired = templates.get("item_acquired",
            "Player has acquired {count} {item_name}.")
        self.tpl_consumed = templates.get("item_consumed",
            "Player has consumed {count} {item_name}.")
    ...
    def evaluate(self, trigger_event: WorldMemoryEvent,
        ...
        item_name = trigger_event.event_subtype
        ...
        if et == "item_acquired":
            narrative = self.tpl_acquired.format(count=count, item_name=item_name)
        else:
            narrative = self.tpl_consumed.format(count=count, item_name=item_name)
        ...
        return InterpretedEvent.create(
            ...
            affects_tags=["type:player", f"event:{et}", f"item:{item_name}"],
            is_ongoing=True,
