"""Items Equipment Evaluator — narrates equipment change frequency."""
...
class ItemsEquipmentEvaluator(PatternEvaluator):

    RELEVANT_TYPES = {"item_equipped"}

    def __init__(self):
        cfg = get_evaluator_config("items_equipment")
        ...
        self.tpl_equipped = templates.get("item_equipped",
            "Player has changed equipment {count} times.")
    ...
    def evaluate(self, trigger_event: WorldMemoryEvent,
        ...
        count = event_store.count_filtered(event_type="item_equipped")
        ...
        narrative = self.tpl_equipped.format(count=count)
        ...
        return InterpretedEvent.create(
            ...
            affects_tags=["type:player", "event:item_equipped"],
            is_ongoing=True,
