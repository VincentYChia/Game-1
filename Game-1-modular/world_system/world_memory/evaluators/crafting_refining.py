"""Crafting Refining Evaluator — narrates refining craft counts."""
...
class CraftingRefiningEvaluator(PatternEvaluator):

    RELEVANT_TYPES = {"craft_attempted"}

    def __init__(self):
        cfg = get_evaluator_config("crafting_refining")
    ...
    def is_relevant(self, event: WorldMemoryEvent) -> bool:
        if event.event_type not in self.RELEVANT_TYPES:
            return False
        return event.context.get("discipline") == "refining"
    ...
    def evaluate(self, trigger_event: WorldMemoryEvent,
        ...
        refining_events = [e for e in recent if e.context.get("discipline") == "refining"]
        count = len(refining_events)
        ...
        if qualities:
            ...
            narrative = (
                f"Player has crafted {count} items in refining, "
                f"{quality_count} at {best_quality} quality."
            )
        else:
            narrative = f"Player has crafted {count} items in refining."
        ...
        return InterpretedEvent.create(
            ...
            affects_tags=["domain:refining", "event:crafting", "type:player"],
        )
