"""Crafting Engineering Evaluator — narrates engineering craft counts."""
...
class CraftingEngineeringEvaluator(PatternEvaluator):

    RELEVANT_TYPES = {"craft_attempted"}

    def __init__(self):
        cfg = get_evaluator_config("crafting_engineering")
    ...
    def is_relevant(self, event: WorldMemoryEvent) -> bool:
        if event.event_type not in self.RELEVANT_TYPES:
            return False
        return event.context.get("discipline") == "engineering"
    ...
    def evaluate(self, trigger_event: WorldMemoryEvent,
        ...
        engineering_events = [e for e in recent if e.context.get("discipline") == "engineering"]
        ...
        if qualities:
            ...
            narrative = (
                f"Player has crafted {count} items in engineering, "
                f"{quality_count} at {best_quality} quality."
            )
        else:
            narrative = f"Player has crafted {count} items in engineering."
        ...
        return InterpretedEvent.create(
            ...
            affects_tags=["domain:engineering", "event:crafting", "type:player"],
        )
